...
const App = () => {
  ...
  const [allSlots, setAllSlots] = useState([
    '로고 배너', '레이어', '헤더배너', '강조 no.1 (#1)', '강조 no.1 (#2)', '강조 no.1 (#3)', '강조 no.3 (랜덤)'
  ]);

  const [banners, setBanners] = useState([
    { id: '1', name: '개편안내 마케팅 캠페인 (장기)', slot: '강조 no.1 (#1)', start: '2026-02-20', end: '2026-03-15', dept: '운영', color: '#FEF3C7', memo: '신규 디자인 반영' },
    { id: '2', name: '보안 자료 업데이트 공지', slot: '강조 no.1 (#2)', start: '2026-03-02', end: '2026-03-10', dept: '콘텐츠', color: '#D1FAE5', memo: '공지사항 링크' },
    { id: '3', name: '연수플러스 특별 프로모션', slot: '강조 no.1 (#3)', start: '2026-03-03', end: '2026-03-15', dept: '마케팅', color: '#EDE9FE', memo: '프로모션 배너' },
    { id: '4', name: '3월 말일 결산 이벤트', slot: '강조 no.1 (#1)', start: '2026-03-12', end: '2026-03-25', dept: '영업', color: '#FFEDD5', memo: '월말 프로모션' },
  ]);

  const [visibleSlots, setVisibleSlots] = useState(
    allSlots.reduce((acc, slot) => ({ ...acc, [slot]: true }), {})
  );
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            onClick={() => {
              ...
              setBanners([...banners, {
                id, name: '새 배너', slot: allSlots[0], start: formatDate(new Date()), end: formatDate(new Date(Date.now() + 86400000 * 4)),
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex items-center px-3 w-full h-full pointer-events-none gap-2">
                                    ...
                                    {banner.dept && (
                                      <span className="text-[9px] px-1 bg-black/10 rounded font-black text-slate-700 whitespace-nowrap">
                                        [{banner.dept}]
                                      </span>
                                    )}
                                    <span className="text-[11px] font-bold text-slate-800 truncate drop-shadow-sm">
                                      {banner.name}
                                    </span>
                                  </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
          <div className="p-4 border-b flex justify-between items-center bg-white flex-wrap gap-4">
            <h2 className="font-bold text-slate-800 flex items-center gap-2 text-base">
              ...
              배너 데이터 상세 설정 <span className="text-blue-500 text-xs px-2 py-0.5 bg-blue-50 rounded-full border border-blue-100 font-semibold ml-1">Total {filteredBanners.length}</span>
            </h2>
